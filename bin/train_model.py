...
class TranslateAE:
    def __init__(
        ...
        def encoder_rna(input_data, nlayer, hidden_frac, reuse=tf.AUTO_REUSE):
            ...
            batch_dim_x = 11
            embed_dim_x = 25
            -> intermediate_dim = 328

            input (n_genes + n_batches): 17271 + 11
            fc1: 328
            fc2: 328
            output (mu, var): 25
            """
            with tf.variable_scope("encoder_x", reuse=tf.AUTO_REUSE):
                self.intermediate_dim = int(
                    math.sqrt((self.input_dim_x + self.batch_dim_x) * self.embed_dim_x)
                    / hidden_frac
                )
                l1 = tf.layers.Dense(
                    self.intermediate_dim, activation=None, name="encoder_x_0"
                )(input_data)
                ...
                for layer_i in range(1, nlayer):
                    l1 = tf.layers.Dense(
                        ...
                        activation=None,
                        name="encoder_x_" + str(layer_i),
                    )(l1)
                    l1 = tf.contrib.layers.layer_norm(
                        inputs=l1, center=True, scale=True
                    )
                    l1 = tf.nn.leaky_relu(l1)
                    l1 = tf.nn.dropout(l1, rate=self.dropout_rate)

                encoder_output_mean = tf.layers.Dense(
                    self.embed_dim_x, activation=None, name="encoder_x_mean"
                )(l1)
                encoder_output_var = tf.layers.Dense(
                    self.embed_dim_x, activation=None, name="encoder_x_var"
                )(l1)
                encoder_output_var = tf.clip_by_value(
                    encoder_output_var, clip_value_min=-2000000, clip_value_max=15
                )
                encoder_output_var = tf.math.exp(encoder_output_var) + 0.0001
                eps = tf.random_normal(
                    (tf.shape(input_data)[0], self.embed_dim_x), 0, 1, dtype=tf.float32
                )
                encoder_output_z = (
                    encoder_output_mean + tf.math.sqrt(encoder_output_var) * eps
                )
        ...
        def decoder_rna(encoded_data, nlayer, hidden_frac, reuse=tf.AUTO_REUSE):
            ...
            fc2: 328
            fc3: 328
            output (scale, dropout, r): n_genes = 17271
            """
            ...
            ## TODO: similarly, here we need to add another dimension for each new input batch factor (where previous batch dimension will be 0 and new dimension will be 1). Add an option of only fine tune weights coming out of this added batch dimension when we input new dataset.
            with tf.variable_scope("decoder_x", reuse=tf.AUTO_REUSE):
                ...
                l1 = tf.layers.Dense(
                    self.intermediate_dim, activation=None, name="decoder_x_0"
                )(encoded_data)
                l1 = tf.contrib.layers.layer_norm(inputs=l1, center=True, scale=True)
                l1 = tf.nn.leaky_relu(l1)
                for layer_i in range(1, nlayer):
                    ...
                    l1 = tf.nn.leaky_relu(l1)
                px = tf.layers.Dense(
                    self.intermediate_dim, activation=tf.nn.relu, name="decoder_x_px"
                )(l1)
                px_scale = tf.layers.Dense(
                    self.input_dim_x,
                    activation=tf.nn.softmax,
                    name="decoder_x_px_scale",
                )(px)
                px_dropout = tf.layers.Dense(
                    self.input_dim_x, activation=None, name="decoder_x_px_dropout"
                )(px)
                px_r = tf.layers.Dense(
                    self.input_dim_x, activation=None, name="decoder_x_px_r"
                )(
                    px
                )  # , use_bias=False
        ...
        def encoder_atac(
            input_data, nlayer, hidden_frac, chr_list, reuse=tf.AUTO_REUSE
        ):
            """
            ...
            with tf.variable_scope("encoder_y", reuse=tf.AUTO_REUSE):
                ...
                for chri in chr_list.keys():
                    dic_intermediate_dim[chri] = int(
                        math.sqrt(
                            (len(chr_list[chri]) + self.batch_dim_y) * self.embed_dim_y
                        )
                        / hidden_frac
                    )
                    dic_l1[chri] = tf.layers.Dense(
                        dic_intermediate_dim[chri],
                        activation=None,
                        name="encoder_y_0" + str(chri),
                    )(
                        tf.gather(
                            input_data,
                            chr_list[chri]
                            + list(
                                range(
                                    self.input_dim_y - self.batch_dim_y,
                                    self.input_dim_y,
                                )
                            ),
                            axis=1,
                    ...
                    dic_l1[chri] = tf.contrib.layers.layer_norm(
                        inputs=dic_l1[chri], center=True, scale=True
                    )
                    ...
                    for layer_i in range(1, nlayer):
                        dic_l1[chri] = tf.layers.Dense(
                            dic_intermediate_dim[chri],
                            activation=None,
                            name="encoder_y_" + str(layer_i) + "_" + str(chri),
                        )(dic_l1[chri])
                        dic_l1[chri] = tf.contrib.layers.layer_norm(
                            inputs=dic_l1[chri], center=True, scale=True
                        )
                        dic_l1[chri] = tf.nn.leaky_relu(dic_l1[chri])
                        dic_l1[chri] = tf.nn.dropout(
                            dic_l1[chri], rate=self.dropout_rate
                        )
                    dic_l2[chri] = tf.layers.Dense(
                        self.embed_dim_y,
                        activation=None,
                        name="encoder_y_output_" + str(chri),
                    )(dic_l1[chri])
                    dic_l2[chri] = tf.contrib.layers.layer_norm(
                        inputs=dic_l2[chri], center=True, scale=True
                    )
                ...
                encoder_output_mean = tf.layers.Dense(
                    self.embed_dim_y, activation=None, name="encoder_y_mean"
                )(l2_concatenate)
                encoder_output_var = tf.layers.Dense(
                    self.embed_dim_y, activation=None, name="encoder_y_var"
                )(l2_concatenate)
                encoder_output_var = tf.clip_by_value(
                    encoder_output_var, clip_value_min=-2000000, clip_value_max=15
                )
                eps = tf.random_normal(
                    (tf.shape(input_data)[0], self.embed_dim_y), 0, 1, dtype=tf.float32
                )
                encoder_output_z = (
                    encoder_output_mean + tf.math.exp(0.5 * encoder_output_var) * eps
                )
                return encoder_output_mean, encoder_output_var, encoder_output_z

        def decoder_atac(
            encoded_data, nlayer, hidden_frac, chr_list, reuse=tf.AUTO_REUSE
        ):
            """
            ...
            with tf.variable_scope("decoder_y", reuse=tf.AUTO_REUSE):
                l1 = tf.layers.Dense(
                    self.embed_dim_y * 22, activation=None, name="decoder_y_0"
                )(encoded_data)
                l1 = tf.contrib.layers.layer_norm(inputs=l1, center=True, scale=True)
                ...
                py = []
                for chri in chr_list.keys():
                    dic_intermediate_dim_decode[chri] = int(
                        math.sqrt(
                            (len(chr_list[chri]) + self.batch_dim_y) * self.embed_dim_y
                        )
                        / hidden_frac
                    )
                    dic_l2_decode[chri] = tf.layers.Dense(
                        dic_intermediate_dim_decode[chri],
                        activation=None,
                        name="decoder_y_1" + str(chri),
                    )(l1)
                    dic_l2_decode[chri] = tf.contrib.layers.layer_norm(
                        inputs=dic_l2_decode[chri], center=True, scale=True
                    )
                    dic_l2_decode[chri] = tf.nn.leaky_relu(dic_l2_decode[chri])
                    dic_l2_decode[chri] = tf.nn.dropout(
                        dic_l2_decode[chri], rate=self.dropout_rate
                    )
                    for layer_i in range(1, nlayer):
                        dic_l2_decode[chri] = tf.layers.Dense(
                            dic_intermediate_dim_decode[chri],
                            activation=None,
                            name="decoder_y_" + str(nlayer + 1) + "_" + str(chri),
                        )(dic_l2_decode[chri])
                        dic_l2_decode[chri] = tf.contrib.layers.layer_norm(
                            inputs=dic_l2_decode[chri], center=True, scale=True
                        )
                    ...
                    dic_l1_decode[chri] = tf.layers.Dense(
                        len(chr_list[chri]),
                        activation=tf.nn.sigmoid,
                        name="decoder_y_output" + str(chri),
                    )(dic_l2_decode[chri])
        ...
        def translator_rnatoatac(encoded_data, trans_ver, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope("translator_xy", reuse=tf.AUTO_REUSE):
                if trans_ver == "linear":
                    translator_output = tf.layers.Dense(
                        self.embed_dim_y, activation=None, name="translator_xy_1"
                    )(encoded_data)
                elif trans_ver == "1l":
                    translator_output = tf.layers.Dense(
                        self.embed_dim_y,
                        activation=tf.nn.leaky_relu,
                        name="translator_xy_1",
                    )(encoded_data)
                elif trans_ver == "2l":
                    l1 = tf.layers.Dense(
                        self.embed_dim_y,
        ...
        def translator_atactorna(encoded_data, trans_ver, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope("translator_yx", reuse=tf.AUTO_REUSE):
                if trans_ver == "linear":
                    translator_output = tf.layers.Dense(
                        self.embed_dim_x, activation=None, name="translator_yx_1"
                    )(encoded_data)
                elif trans_ver == "1l":
                    translator_output = tf.layers.Dense(
                        self.embed_dim_x,
                        activation=tf.nn.leaky_relu,
                        name="translator_yx_1",
                    )(encoded_data)
                elif trans_ver == "2l":
                    l1 = tf.layers.Dense(
                        self.embed_dim_x,
        ...
        self.px_z_m, self.px_z_v, self.encoded_x = encoder_rna(
            tf.concat([self.input_x, self.batch_x], 1), self.nlayer, self.hidden_frac
        )

        z = tf.truncated_normal(tf.shape(self.px_z_m), stddev=1.0)
        self.py_z_m, self.py_z_v, self.encoded_y = encoder_atac(
            tf.concat([self.input_y, self.batch_y], 1),
            self.nlayer,
            self.hidden_frac,
            self.chr_list,
        )
        ...
        self.px_scale, self.px_dropout, self.px_r = decoder_rna(
            tf.concat([self.encoded_x, self.batch_x], 1), self.nlayer, self.hidden_frac
        )
        # per default: self.dispersion = "genebatch"
        if self.dispersion == "genebatch":
            self.px_r = tf.layers.Dense(
                self.input_dim_x, activation=None, name="px_r_genebatch_x"
            )(self.batch_x)

        self.px_r = tf.clip_by_value(
            self.px_r, clip_value_min=-2000000, clip_value_max=15
        )
        ...
        self.pi_theta_log = -self.px_dropout + tf.multiply(
            self.px_r, (self.log_theta_eps - self.log_theta_mu_eps)
        )

        self.case_zero = tf.nn.softplus(self.pi_theta_log) - self.softplus_pi
        self.mul_case_zero = tf.multiply(
            tf.dtypes.cast(self.input_x < 1e-8, tf.float32), self.case_zero
        )
        ...
        self.case_non_zero = (
            ...
            + self.pi_theta_log
            + tf.multiply(
                self.input_x, (tf.log(self.reconstr_x + 1e-8) - self.log_theta_mu_eps)
            )
            + tf.lgamma(self.input_x + self.px_r)
            - tf.lgamma(self.px_r)
            - tf.lgamma(self.input_x + 1)
        )
        self.mul_case_non_zero = tf.multiply(
            tf.dtypes.cast(self.input_x > 1e-8, tf.float32), self.case_non_zero
        )
        ...
        self.kld_loss_x = (
            tf.reduce_mean(
                ...
                * (
                    tf.reduce_sum(
                        -tf.math.log(self.px_z_v)
                        + self.px_z_v
                        + tf.math.square(self.px_z_m)
                        - 1,
                        axis=1,
                    )
                )
        ...
        self.py = decoder_atac(
            tf.concat([self.encoded_y, self.batch_y], 1),
            self.nlayer,
            self.hidden_frac,
            self.chr_list,
        )
        ...
        self.kld_loss_y = (
            tf.reduce_mean(
                ...
                * (
                    tf.reduce_sum(
                        -self.py_z_v
                        + tf.math.exp(self.py_z_v)
                        + tf.math.square(self.py_z_m)
                        - 1,
                        axis=1,
                    )
                )
        ...
        self.translator_encoded_x = translator_atactorna(
            self.encoded_y, trans_ver=self.trans_ver
        )
        self.translator_encoded_y = translator_rnatoatac(
            self.encoded_x, trans_ver=self.trans_ver
        )

        ## translate to scRNA
        (
            self.px_scale_translator,
            self.px_dropout_translator,
            self.px_r_translator,
        ) = decoder_rna(
            tf.concat([self.translator_encoded_x, self.batch_x], 1),
            self.nlayer,
            self.hidden_frac,
        )
        if self.dispersion == "genebatch":
            self.px_r_translator = tf.layers.Dense(
                self.input_dim_x, activation=None, name="translator_yx_px_r_genebatch"
            )(self.batch_y)

        self.px_r_translator = tf.clip_by_value(
            self.px_r_translator, clip_value_min=-2000000, clip_value_max=15
        )
        self.px_r_translator = tf.math.exp(self.px_r_translator)
        self.translator_reconstr_x = tf.transpose(
            tf.transpose(self.px_scale_translator) * self.libsize_x
        )

        ## loss
        self.softplus_pi_translator = tf.nn.softplus(
            -self.px_dropout_translator
        )  #  uses log(sigmoid(x)) = -softplus(-x)
        self.log_theta_eps_translator = tf.log(self.px_r_translator + 1e-8)
        self.log_theta_mu_eps_translator = tf.log(
            self.px_r_translator + self.translator_reconstr_x + 1e-8
        )
        self.pi_theta_log_translator = -self.px_dropout_translator + tf.multiply(
            self.px_r_translator,
            (self.log_theta_eps_translator - self.log_theta_mu_eps_translator),
        )

        self.case_zero_translator = (
            tf.nn.softplus(self.pi_theta_log_translator) - self.softplus_pi_translator
        )
        self.mul_case_zero_translator = tf.multiply(
            tf.dtypes.cast(self.input_x < 1e-8, tf.float32), self.case_zero_translator
        )
        ...
        self.case_non_zero_translator = (
            ...
            + tf.multiply(
                self.input_x,
                (
                    tf.log(self.translator_reconstr_x + 1e-8)
                    - self.log_theta_mu_eps_translator
                ),
            )
        ...
        self.mul_case_non_zero_translator = tf.multiply(
            tf.dtypes.cast(self.input_x > 1e-8, tf.float32),
            self.case_non_zero_translator,
        )

        self.res_translator = (
            self.mul_case_zero_translator + self.mul_case_non_zero_translator
        )
        self.translator_loss_x = -tf.reduce_mean(
            tf.reduce_sum(self.res_translator, axis=1)
        )

        ## translate to scATAC
        self.py_translator = decoder_atac(
            tf.concat([self.translator_encoded_y, self.batch_y], 1),
            self.nlayer,
            self.hidden_frac,
            self.chr_list,
        )
        self.translator_reconstr_y = tf.transpose(
            tf.transpose(self.py_translator) * self.libsize_y
        )

        ## loss
        self.translator_loss_y = (
            bce(self.input_y, self.translator_reconstr_y) * self.input_dim_y
        )

        ## optimizers
        self.train_vars_x = [
            var for var in tf.trainable_variables() if "_x" in var.name
        ]
        self.train_vars_y = [
            var for var in tf.trainable_variables() if "_y" in var.name
        ]
        self.train_vars_trans_x = [
            var for var in tf.trainable_variables() if "translator_yx" in var.name
        ]
        self.train_vars_trans_y = [
            var for var in tf.trainable_variables() if "translator_xy" in var.name
        ]
        ...
        self.optimizer_x = tf.train.AdamOptimizer(
            learning_rate=self.learning_rate_x, epsilon=0.01
        ).minimize(self.loss_x, var_list=self.train_vars_x)
        self.optimizer_y = tf.train.AdamOptimizer(
            learning_rate=self.learning_rate_y, epsilon=0.00001
        ).minimize(self.loss_y, var_list=self.train_vars_y)
        self.optimizer_trans_x = tf.train.AdamOptimizer(
            learning_rate=self.learning_rate_yx
        ).minimize(self.translator_loss_x, var_list=self.train_vars_trans_x)
        self.optimizer_trans_y = tf.train.AdamOptimizer(
            learning_rate=self.learning_rate_xy
        ).minimize(self.translator_loss_y, var_list=self.train_vars_trans_y)
        ...
        for iter in range(1, 2000):
            ...
            if sep_train_index == 1:
                if neptune_logger is not None:
                    neptune_logger["sys/tags"].add("scATAC VAE")
                if n_iter_step1 < nepoch_klstart_y:
                    kl_weight_y_update = 0
                else:
                    kl_weight_y_update = min(
                        1.0, (n_iter_step1 - nepoch_klstart_y) / float(nepoch_warmup_y)
                    )  # 50
                ...
                for batch_id in range(0, data_y.shape[0] // batch_size + 1):
                    data_y_i = data_y[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_y.shape[0]
                        ),
                    ].todense()
                    batch_y_i = batch_y[
                        (batch_size * batch_id) : min(
                    ...
                    self.sess.run(
                        self.optimizer_y,
                        feed_dict={
                            self.input_x: np.zeros(
                                shape=(data_y_i.shape[0], data_x.shape[1]),
                                dtype=np.int32,
                            ),
                            self.input_y: data_y_i,
                            self.batch_x: np.zeros(
                                shape=(batch_y_i.shape[0], batch_x.shape[1]),
                                dtype=np.int32,
                            ),
                            self.batch_y: batch_y_i,
                            self.kl_weight_x: 0.0,
                            self.kl_weight_y: kl_weight_y_update,
                        },
                    )
                ...
                for batch_id in range(0, data_y_val.shape[0] // batch_size + 1):
                    data_y_vali = data_y_val[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_y_val.shape[0]
                        ),
                    ].todense()
                    batch_y_vali = batch_y_val[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_y_val.shape[0]
                        ),
                    ].todense()
                    loss_reconstruct_y_val_i, loss_kl_y_val_i = self.get_losses_atac(
                        np.zeros(
                            shape=(data_y_vali.shape[0], data_x.shape[1]),
                            dtype=np.int32,
                        ),
                        data_y_vali,
                        np.zeros(
                            shape=(batch_y_vali.shape[0], batch_x.shape[1]),
                            dtype=np.int32,
                        ),
                        batch_y_vali,
                        0.0,
                ...
                if neptune_logger is not None:
                    neptune_logger["training/reconstruct_ATAC_val_loss"].log(
                        loss_val_check
                    )
            ...
            if sep_train_index == 2:
                if neptune_logger is not None:
                    neptune_logger["sys/tags"].add("scRNA VAE")
                if n_iter_step2 < nepoch_klstart_x:
                    kl_weight_x_update = 0
                else:
                    kl_weight_x_update = min(
                        1.0, (n_iter_step2 - nepoch_klstart_x) / float(nepoch_warmup_x)
                    )
                ...
                for batch_id in range(0, data_x.shape[0] // batch_size + 1):
                    data_x_i = data_x[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_x.shape[0]
                        ),
                    ].todense()
                    batch_x_i = batch_x[
                        (batch_size * batch_id) : min(
                    ...
                    self.sess.run(
                        self.optimizer_x,
                        feed_dict={
                            self.input_x: data_x_i,
                            self.input_y: np.zeros(
                                shape=(data_x_i.shape[0], data_y.shape[1]),
                                dtype=np.int32,
                            ),
                            self.batch_x: batch_x_i,
                            self.batch_y: np.zeros(
                                shape=(batch_x_i.shape[0], batch_y.shape[1]),
                                dtype=np.int32,
                            ),
                ...
                for batch_id in range(0, data_x_val.shape[0] // batch_size + 1):
                    data_x_vali = data_x_val[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_x_val.shape[0]
                        ),
                    ].todense()
                    batch_x_vali = batch_x_val[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_x_val.shape[0]
                        ),
                    ].todense()
                    loss_reconstruct_x_val_i, loss_kl_x_val_i = self.get_losses_rna(
                        data_x_vali,
                        np.zeros(
                            shape=(data_x_vali.shape[0], data_y.shape[1]),
                            dtype=np.int32,
                        ),
                        batch_x_vali,
                        np.zeros(
                            shape=(batch_x_vali.shape[0], batch_y.shape[1]),
                            dtype=np.int32,
                        ),
                        kl_weight_x_update,
                        0.0,
                ...
                if neptune_logger is not None:
                    neptune_logger["training/reconstruct_RNA_val_loss"].log(
                        loss_val_check
                    )
            ...
            if sep_train_index == 3:
                if neptune_logger is not None:
                    neptune_logger["sys/tags"].add("ATAC->RNA translator")
                ...
                for batch_id in range(0, data_x_co.shape[0] // batch_size + 1):
                    data_x_i = data_x_co[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_x_co.shape[0]
                        ),
                    ].todense()
                    data_y_i = data_y_co[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_y_co.shape[0]
                        ),
                    ].todense()
                    batch_x_i = batch_x_co[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_x_co.shape[0]
                        ),
                    ].todense()
                    batch_y_i = batch_y_co[
                        (batch_size * batch_id) : min(
                            batch_size * (batch_id + 1), data_y_co.shape[0]
                        ),
                    ].todense()
                    self.sess.run(
                        self.optimizer_trans_x,
                        feed_dict={
                            self.input_x: data_x_i,
                            self.input_y: data_y_i,
                            self.batch_x: batch_x_i,
                ...
                for batch_id in range(0, data_x_val.shape[0] // batch_size + 1):
                    ...
                    loss_translator_x_val_i = self.get_losses_translatetorna(
                        data_x_vali, data_y_vali, batch_x_vali, batch_y_vali
                    )
                ...
                if neptune_logger is not None:
                    neptune_logger["training/translate_to_RNA_val_loss"].log(
                        loss_val_check
                    )
            ...
            if sep_train_index == 4:
                if neptune_logger is not None:
                    neptune_logger["sys/tags"].add("RNA->ATAC translator")
                ...
                for batch_id in range(0, data_x_val.shape[0] // batch_size + 1):
                    ...
                    loss_translator_y_val_i = self.get_losses_translatetoatac(
                        data_x_vali, data_y_vali, batch_x_vali, batch_y_vali
                    )
                ...
                if neptune_logger is not None:
                    neptune_logger["training/translate_to_ATAC_val_loss"].log(
                        loss_val_check
                    )
            ...
            if (iter + 1) % 1 == 0:  # check every epoch
                ...
                if loss_val_check < loss_val_check_best:
                    ...
                    saver.save(
                        self.sess,
                        output_model + "_step" + str(sep_train_index) + "/mymodel",
                    )
                    loss_val_check_best = loss_val_check
                    last_improvement = 0
                else:
                ...
                if len(loss_val_check_list) > 1:
                    ...
                    if (
                        stop_decision
                        or len(iter_list) == my_epochs_max[sep_train_index] - 1
                    ):
                        ...
                        saver.restore(
                            ...
                            tf.train.latest_checkpoint(
                                output_model + "_step" + str(sep_train_index) + "/"
                            ),
                        )
                        print(
                            "step"
                            + str(sep_train_index)
                            + " reached minimum, switching to next"
                        )
                        last_improvement = 0
                        loss_val_check_list = []
    ...
    def load(self, output_model):
        ...
        if os.path.exists(output_model + "_step4/"):
            ...
            saver.restore(
                self.sess, tf.train.latest_checkpoint(output_model + "_step4/")
            )
    ...
    def predict_embedding(self, data_x, data_y, batch_x, batch_y):
        ...
        return self.sess.run(
            [
                self.encoded_x,
                self.translator_encoded_x,
                self.encoded_y,
                self.translator_encoded_y,
            ],
            feed_dict={
                self.input_x: data_x,
                self.input_y: data_y,
    ...
    def predict_reconstruction(self, data_x, data_y, batch_x, batch_y):
        ...
        return self.sess.run(
            [
                self.translator_reconstr_x,
                self.translator_reconstr_y,
                self.reconstr_x,
                self.reconstr_y,
            ],
            feed_dict={
    ...
    def predict_rnanorm_translation(self, input_dim_x, data_y, batch_y):
        ...
        return self.sess.run(
            ...
            feed_dict={
                self.input_x: np.zeros(
                    shape=(data_y.shape[0], input_dim_x), dtype=np.int32
                ),
                self.input_y: data_y,
                self.batch_x: batch_y,
                self.batch_y: batch_y,
            },
        )
    ...
    def predict_atacnorm_translation(self, data_x, batch_x, input_dim_y):
        ...
        return self.sess.run(
            ...
            feed_dict={
                self.input_x: data_x,
                self.input_y: np.zeros(
                    shape=(data_x.shape[0], input_dim_y), dtype=np.int32
                ),
                self.batch_x: batch_x,
                self.batch_y: batch_x,
            },
        )

    def get_losses_rna(
        self, data_x, data_y, batch_x, batch_y, kl_weight_x, kl_weight_y
    ):
        """
        return scRNA reconstruction loss
        """
        return self.sess.run(
            [self.reconstr_loss_x, self.kld_loss_x],
            feed_dict={
                self.input_x: data_x,
                self.input_y: data_y,
                self.batch_x: batch_x,
    ...
    def get_losses_atac(
        ...
        """
        return self.sess.run(
            [self.reconstr_loss_y, self.kld_loss_y],
            feed_dict={
                self.input_x: data_x,
                self.input_y: data_y,
                self.batch_x: batch_x,
