...
def FOSCTTM(domain1, domain2, output_full=False):
    ...
    fraction_2to1 = []
    for i in range(n):
        fraction_1to2.append(
            np.sum(distances_1to2[i, i] > distances_1to2[i, :]) / (n - 1)
        )
        fraction_2to1.append(
            np.sum(distances_2to1[i, i] > distances_2to1[i, :]) / (n - 1)
        )
...
def plot_auroc_perpeak(matrix_true, matrix_pred):
    ...
    fpr, tpr, _thresholds = metrics.roc_curve(
        matrix_true.flatten(), matrix_pred.flatten()
    )
...
def plot_cor_pergene(x, y, logscale, normlib):
    ...
    if logscale:
        x = np.log1p(x)
        y = np.log1p(y)
    pearson_r_flatten, pearson_p_flatten = scipy.stats.pearsonr(
        x.flatten(), y.flatten()
    )
    pearson_r_list = []
    npos = []
